...
class HtmlReport implements IReport {
	...
	public function __construct ($runner, $outputHandler = null, $traceRedirected = true) {
		...
		if (null === $outputHandler) {
			...
			$this->setHandler(new HxClosure($this, '_handler'));
		} else {
	...
	public function formatStack ($stack, $addNL = true) {
		...
		$_g = 0;
		#/Users/ut/haxe/haxe_libraries/utest/1.9.5/github/aca590b256e0879cd7fbfcd47ca4f3a0a0ce1845/src/utest/ui/text/HtmlReport.hx:145: lines 145-160
		$_g1 = \Array_hx::wrap(explode("\x0A", CallStack::toString($stack)));
		#/Users/ut/haxe/haxe_libraries/utest/1.9.5/github/aca590b256e0879cd7fbfcd47ca4f3a0a0ce1845/src/utest/ui/text/HtmlReport.hx:145: lines 145-160
		while ($_g < $_g1->length) {
			...
			if ($part === $last) {
				#/Users/ut/haxe/haxe_libraries/utest/1.9.5/github/aca590b256e0879cd7fbfcd47ca4f3a0a0ce1845/src/utest/ui/text/HtmlReport.hx:155: characters 50-59
				$count = $count + 1;
					...
					while ($_g6->head !== null) {
						...
						switch ($val->index) {
							...
							case 1:
								...
								$messages = ($messages??'null') . ((($indents(2)??'null') . "line: " . ($val->params[1]->lineNumber??'null') . ", " . ($val->params[0]??'null') . ($newline??'null'))??'null');
								break;
							case 2:
								...
								$messages = ($messages??'null') . ((($indents(2)??'null') . (\Std::string($val->params[0])??'null') . ($dumpStack($val->params[1])??'null') . ($newline??'null'))??'null');
								break;
							case 3:
							...
							case 5:
								...
								$messages = ($messages??'null') . ((($indents(2)??'null') . "missed async calls: " . ($val->params[0]??'null') . ($dumpStack($val->params[1])??'null') . ($newline??'null'))??'null');
								break;
							case 6:
							...
							case 7:
								...
								$messages = ($messages??'null') . ((($indents(2)??'null') . ($val->params[0]??'null') . ($newline??'null'))??'null');
								break;
							case 8:
								...
								if (($reason !== null) && ($reason !== "")) {
									...
									$messages = ($messages??'null') . ((($indents(2)??'null') . (("With reason: " . ($reason??'null'))??'null') . ($newline??'null'))??'null');
								}
	...
	public function redirectTrace () {
		...
		Log::$trace = new HxClosure($this, '_trace');
	}
