...
class Runner {
	...
	public function isTestFixtureName ($name, $prefixes, $pattern = null, $globalPattern = null) {
		...
		if (($pattern === null) && ($globalPattern === null)) {
			...
			while ($_g < $prefixes->length) {
				#/Users/ut/haxe/haxe_libraries/utest/1.9.5/github/aca590b256e0879cd7fbfcd47ca4f3a0a0ce1845/src/utest/Runner.hx:227: characters 11-17
				$prefix = ($prefixes->arr[$_g] ?? null);
				#/Users/ut/haxe/haxe_libraries/utest/1.9.5/github/aca590b256e0879cd7fbfcd47ca4f3a0a0ce1845/src/utest/Runner.hx:227: lines 227-231
				$_g = $_g + 1;
	...
	public function runFixture ($fixture) {
		...
		$handler->onComplete->add(new HxClosure($this, 'testComplete'));
		...
		$handler->onPrecheck->add(new HxClosure($this->onPrecheck, 'dispatch'));
	...
	public function runNext ($finishedHandler = null) {
		...
		while ($_g1 < $_g) {
			...
			if (!$handler->finished) {
				...
				$handler->onComplete->add(new HxClosure($this, 'runNext'));
